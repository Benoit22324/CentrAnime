...
const updatedAnimes: AnimeWithGenre[] = [
    {
        id: "1",
        main_title: "Anime 1",
        en_title: "Anime 1 EN",
...
class InMemoryAnimeRepository implements AnimeRepositoryInterface {
    async getAnimesByPage(selectedPage: number, maxItems: number, searchName: string | null, filterGenre: string | null): Promise<GetAnimesByPageOutputs | null> {
        if (selectedPage > 0 && maxItems > 3) return {
            animes: [],
            total: 0
        }

        if (searchName && filterGenre) {
            const filteredList = updatedAnimes.filter(a => (a.main_title.toLocaleLowerCase().includes(searchName.toLocaleLowerCase()) || a.en_title.toLocaleLowerCase().includes(searchName.toLocaleLowerCase())) && a.animeGenres.some(g => g.genre.genreName.toLocaleLowerCase() === filterGenre.toLocaleLowerCase()))
...
export default InMemoryAnimeRepository;